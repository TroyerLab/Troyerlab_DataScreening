function []=write_batch_based_on_classifier(classifier_filename,classifier_filepath,numbers_include,numbers_exclude,varargin)
%% Syntax
%
...
for i=1:no_files
    mark=classifier(i).classification_label;    
    if ~isempty(find(mark==numbers_include,1)) 
        [~,name,ext]=fileparts(classifier(i).filename);
        filelist=[filelist;[name ext]];
    elseif isempty(find(mark==numbers_exclude,1))
        error(['The mark for the file ' classifier.filename ' is listed neither in the included or the excluded list.'])
    end   
end
