function [screening_ftrs,classification_labels_present]=aggregate_screening_ftrs(is_batch,fname,fpath,selected_ftrs_file,selected_ftrs_path)
%% Syntax
%
...
if ~isempty(selected_ftrs_path)
    if ~strcmpi(selected_ftrs_path(end),filesep)
        selected_ftrs_path=[selected_ftrs_path,filesep];
    end
end

%% Body of the function

if ~(selected_ftrs_file==0)
    selected_ftrs_fullfile=[selected_ftrs_path selected_ftrs_file];
    load(selected_ftrs_fullfile) % loads variables called features and prc
...
if is_batch==1
    ...
    for i=1:no_files 
        ...
        if ~selected_ftrs_file==0 % in case screening_ftrs_dists have been loaded
            screening_ftrs=[];
...
elseif is_batch==0
   ...
   error('Incorrect input for is_batch') 
end
